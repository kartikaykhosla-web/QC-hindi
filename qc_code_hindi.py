...
PROMPT_VERSION_HI = "2026-04-11-stabilize-1"
...
def visible_text_signature(text: str) -> str:
    ...
    for unit in split_grapheme_like_units(normalized):
        base = "".join(ch for ch in unit if unicodedata.category(ch)[0] != "M")
        if not base:
            continue
        marks = []
        for ch in unit:
            if unicodedata.category(ch)[0] == "M":
                if not marks or marks[-1] != ch:
                    marks.append(ch)
        units.append(base + "".join(marks))
    normalized = "".join(units)
...
def is_punctuation_only_correction(original: str, corrected: str) -> bool:
    ...
    if not original_signature or not corrected_signature:
        return False
    return original_signature == corrected_signature and normalize_for_equality(original) != normalize_for_equality(corrected)
...
def sanitized_word_token_hi(token: str) -> str:
    cleaned_units = []
    for unit in split_grapheme_like_units(_unicode_norm(token)):
        base = "".join(ch for ch in unit if unicodedata.category(ch)[0] != "M")
        if not base:
            continue
        marks = []
        for ch in unit:
            if unicodedata.category(ch)[0] == "M":
                if not marks or marks[-1] != ch:
                    marks.append(ch)
        cleaned_units.append(base + "".join(marks))
